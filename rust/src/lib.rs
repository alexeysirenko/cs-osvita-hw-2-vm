...
pub fn compute(memory: &mut [u8; 256]) {
    let mut registers: [u8; 3] = [MEMORY_SHIFT as u8, 0, 0];

    loop {
        let pc = registers[0] as usize;
        let op = memory[pc];
...
fn load(memory: &[u8; 256], registers: &mut [u8; 3], cmd_idx: usize) -> usize {
    const CMD_LEN: usize = 3;
    let reg_idx = get_cmd(memory, cmd_idx + 1) as usize;
    let mem_addr = get_cmd(memory, cmd_idx + 2) as usize;
    assert!(reg_idx == R1 || reg_idx == R2);
    registers[reg_idx] = get_mem(memory, mem_addr);
    cmd_idx + CMD_LEN
}

fn store(memory: &mut [u8; 256], registers: &[u8; 3], cmd_idx: usize) -> usize {
    const CMD_LEN: usize = 3;
    let reg_idx = get_cmd(memory, cmd_idx + 1) as usize;
    let mem_addr = get_cmd(memory, cmd_idx + 2) as usize;
    assert!(reg_idx == R1 || reg_idx == R2);
...
fn arithmetic_op(
    ...
    const CMD_LEN: usize = 3;
    let reg1 = get_cmd(memory, cmd_idx + 1) as usize;
    let reg2 = get_cmd(memory, cmd_idx + 2) as usize;
...
fn arithmetic_immediate_op(
    ...
    assert!(reg1 == R1 || reg1 == R2);
    registers[reg1] = op(registers[reg1], value);
    cmd_idx + CMD_LEN
}

fn jump(memory: &[u8; 256], cmd_idx: usize) -> usize {
    let jump_to = get_cmd(memory, cmd_idx + 1) as usize;
    assert!(jump_to >= MEMORY_SHIFT && jump_to < memory.len());
    jump_to
}

// branch if equa to zero is an offset, not absolute value
fn beqz(memory: &[u8; 256], registers: &[u8; 3], cmd_idx: usize) -> usize {
    const CMD_LEN: usize = 3;
    let reg1 = get_cmd(memory, cmd_idx + 1) as usize;
    let offset = get_cmd(memory, cmd_idx + 2) as usize;
    assert!(reg1 == R1 || reg1 == R2);
    if registers[reg1] == 0 {
